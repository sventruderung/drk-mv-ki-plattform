...
ändert. Es installiert nur die in PIPES gelisteten und tatsächlich vorhandenen
Pipe-Dateien.
...
# Pipes mit Anzeigename (erscheint im Modell-Dropdown). White-Label ohne
# content-Pipe (content-service ist in dieser Variante nicht enthalten).
# Nur vorhandene Dateien werden installiert.
PIPES = [
...
for fid, name, fn in PIPES:
    ...
    vals = {"id": fid, "user_id": uid, "name": name, "type": "pipe", "content": content,
            "meta": json.dumps({"description": "Pipe aus " + fn, "manifest": {}}),
...
def main() -> None:
    print("=== KI-Plattform — Open-WebUI-Pipes installieren (direkt in die DB) ===\n")
